<div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto" wire.poll>
    <div class="grid md:grid-cols-2 gap-4">
        ...
        <div class="max-w-4xl mx-auto card lg:card-side bg-base-100 shadow-md max-h-xl mb-4 mt-4">
            ...
            <div class="card-body">
                ...
                <x-form wire:submit="validation">
                    @if ($carton->type === 'SOLID')
                    @if ($completed)
                    <x-input class="input input-bordered input-primary focus:ring-primary focus:outline-primary" autofocus label="Polybag barcode" wire:model="polybagForm.polybag_code" hint="Please scan polybag barcode here." disabled />
                    @else
                    <x-input class="input input-bordered input-primary focus:ring-primary focus:outline-primary" autofocus label="Polybag barcode" wire:model="polybagForm.polybag_code" hint="Please scan polybag barcode here." />
                    @endif
                    @elseif ($carton->type === 'RATIO')
                    @if ($completed)
