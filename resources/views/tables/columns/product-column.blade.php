...
$specification = collect($record->product->specification);
$value = $specification->implode('value', ' ');

$states->push($record->product->name . ' ' . $value );
}
...
<div {{
    ...
    <p @class([ 'text-sm text-gray-500 dark:text-gray-400' , 'whitespace-normal'=> $canWrap,
        ])
        >
        {{ $descriptionAbove }}
    </p>
    ...
    <{{ $isListWithLineBreaks ? 'ul' : 'div' }} @class([ 'list-inside list-disc'=> $isBulleted,
        ...
        <{{ $isListWithLineBreaks ? 'li' : 'div' }} @if ($itemIsCopyable) x-on:click="
                                window.navigator.clipboard.writeText(@js($copyableState))
                                $tooltip(@js($copyMessage), {
                                    theme: $store.theme,
                                    timeout: @js($copyMessageDuration),
                                })
                            " @endif @class([ 'flex'=> ! $isBulleted,
            'max-w-max' => ! $isBadge,
            'w-max' => $isBadge,
            ...
            @else
            <div @class([ 'fi-ta-text-item inline-flex items-center gap-1.5' , 'group/item'=> $url,
                match ($color) {
                null => null,
                'gray' => 'fi-color-gray',
                ...
                <span @class([ 'fi-ta-text-item-label' , 'group-hover/item:underline group-focus-visible/item:underline'=> $url,
                    match ($size) {
        ...
        <{{ $isListWithLineBreaks ? 'li' : 'div' }} class="text-sm text-gray-500 dark:text-gray-400">
